# Guide d'utilisation — Mode asynchrone

> [⇐ Retour à l'index](../README.md)

Ce guide montre un flux d'utilisation complet en mode asynchrone : recherche d'un client, création d'un client, création d'un produit, création d'un devis puis transformation en facture.
...
```python
page = await client.customers.list_customers(search="ACME", limit=10)
...
created_customer = await client.customers.add(customer)
...
created_item = await client.items.add(item)
...
```python
doc_types = await client.document_types.list_document_types()
quote_type = next((d for d in (doc_types.elements or []) if d.label and ("devis" in d.label.lower() or "quote" in d.label.lower())), None)
line_types = await client.document_line_types.list_document_line_types()
...
created_doc = await client.documents.add(doc)
...
```python
invoice = await client.documents.transform_to_invoice(created_doc.id)
print('Invoice id:', invoice.id)

# (optionnel) finaliser
finalized = await client.documents.finalize(invoice.id)
```
...
try:
    await client.customers.add(customer)
...
[↑ Retour en haut](#guide-dutilisation--mode-asynchrone)
