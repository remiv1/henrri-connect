...
class TestSyncDocumentLineTypes:   # pylint: disable=R0903
    """Tests du sous-client document_line_types (synchrone et asynchrone)."""
    def test_list_retourne_types(
        self, sync_client: SyncHenrriClient, mock_http: MagicMock
    ) -> None:
        """Test de la méthode list_document_line_types()."""
...
class TestAsyncDocumentLineTypes:   # pylint: disable=R0903
    """Tests du sous-client document_line_types (synchrone et asynchrone)."""
    async def test_list_retourne_types(
        ...
        mock_async_http.request.return_value = make_response(
            {"elements": [DOCUMENT_LINE_TYPE_JSON]}
        )
